...
import { ExamService, SubjectService, ParentService } from "@/lib/api";
import { Award, TrendingUp, FileText, User } from "lucide-react";
...
function AcademicsContent() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const childExists = children.find((c: any) => c.id === studentId);
            
            if (childExists) {
                setStudentProfile(childExists);
                
                // Fetch student results, all exams, and all subjects
                const [studentResults, allExams, allSubjects] = await Promise.all([
                    ExamService.getStudentResults(studentId!),
                    ExamService.getAll(),
                ...
                }).filter((r: any) => r.status === "RESULTS_PUBLISHED"); 
    ...
    if (!studentId) {
        return <div className="p-12 text-center text-slate-500">Please select a student from the My Children page first.</div>;
    }

    if (loading) {
        return <div className="flex-1 p-12 text-center text-slate-500">Loading academic records...</div>;
    }

    if (!studentProfile) {
        return <div className="p-12 text-center text-red-500">Unauthorized or Student not found.</div>;
    }
        ...
        <div className="flex-1 h-full overflow-y-auto bg-slate-50 dark:bg-zinc-950 p-6 md:p-8">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex items-center gap-4 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-4 shadow-sm">
                    <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center overflow-hidden shrink-0 border-2 border-slate-200">
                        {studentProfile.photoUrl ? <img src={studentProfile.photoUrl} alt="" className="w-full h-full object-cover" /> : <User className="w-6 h-6 text-slate-400" />}
                    </div>
                    <div>
                        <h1 className="text-xl font-bold text-slate-800 dark:text-zinc-100">{studentProfile.name}'s Academics</h1>
                        <p className="text-sm text-slate-500 font-mono mt-0.5">Adm. No: {studentProfile.admissionNumber}</p>
                    </div>
                </div>
                ...
                {results.length === 0 ? (
                    <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-sm p-12 text-center">
                        ...
                        <p className="text-sm text-slate-500 mt-1">There are no published exam results for this student yet.</p>
                    </div>
                ) : (
                    <>
                        {/* Overall Card */}
                        <div className={`bg-gradient-to-r ${overallPct >= 75 ? "from-violet-500 to-purple-600" : overallPct >= 50 ? "from-amber-500 to-orange-600" : "from-red-500 to-rose-600"} text-white rounded-2xl p-6 shadow-lg flex items-center justify-between`}>
                            <div>
                                    ...
                                    return (
                                        <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-4 px-6 py-5 hover:bg-slate-50/50 dark:hover:bg-zinc-800/50 transition-colors">
                                            <div className="flex-1">
                                                ...
                                                {r.remarks && <p className="text-sm mt-3 text-slate-600 dark:text-zinc-400 bg-slate-50 p-2 rounded-lg border border-slate-100">Teacher Note: "{r.remarks}"</p>}
                                            </div>
