...
class ProfileTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final name = user?['name'] ?? (auth.role == 'PARENT' ? 'Parent User' : 'Student User');
    final email = user?['email'] ?? 'user@lmno.edu';
      ...
      child: Column(
        ...
        children: [
          ...
          ClayContainer(
            ...
            child: const Icon(Icons.person, size: 80, color: ClayTheme.primary),
          ),
            ...
            child: ClayButton(
              onPressed: () async {
                ...
                if (confirm == true && context.mounted) {
                  telemetry.logEvent('LOGOUT_TRIGGERED', properties: {'role': auth.role});
                  await auth.logout();
                  if (context.mounted) {
                    context.go('/splash');
                  }
